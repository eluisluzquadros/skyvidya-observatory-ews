...
import { fetchSimulatedData, generateInsight, fetchEventNews, fetchEconomicImpact } from './services/geminiService';

type RightPanel = 'detail' | 'news' | 'economic' | 'ai' | 'chat';
...
const App: React.FC = () => {
    ...
    const filteredData = useMemo(() => {
        ...
        // Time range filtering would apply based on date parsing
        return filtered;
            ...
            <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
                        ...
                        <>
                            ...
                            {insight && (
                                <div style={{
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                                        ...
                                        <span className="font-mono" style={{ fontSize: '0.5rem', color: 'var(--amber)', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 700 }}>
                                            Intel Brief
                                        </span>
                                    </div>
                                    <p className="font-body" style={{ fontSize: '0.6rem', color: 'var(--text-secondary)', lineHeight: 1.5, whiteSpace: 'pre-wrap' }}>
                            ...
                            <div style={{ flex: 1, overflowY: 'auto' }}>
                                <div style={{ padding: '8px 12px', borderBottom: '1px solid var(--border-primary)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                    <span className="font-mono" style={{ fontSize: '0.5rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>
                                        Feed de Eventos
                                    </span>
                                    <span className="font-mono" style={{ fontSize: '0.5rem', color: 'var(--primary)' }}>{filteredData.length}</span>
                                </div>

                                {loading ? (
                                    <div className="font-mono" style={{ padding: 20, textAlign: 'center', fontSize: '0.6rem', color: 'var(--cyan)' }}>
                                        CARREGANDO DADOS...
                                    </div>
                                ) : (
                                    filteredData.map((event, idx) => (
                                        ...
                                        >
                                            ...
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 3 }}>
                                                <span className="font-mono" style={{ fontSize: '0.5rem', color: 'var(--text-muted)' }}>{event.date}</span>
                                                <span className="font-mono" style={{ fontSize: '0.5rem', color: 'var(--amber)' }}>
                ...
                <aside className="hud-border" style={{
                        ...
                        {([
                            ...
                            <button
                                ...
                                style={{
                                    ...
                                    fontSize: '0.45rem', textTransform: 'uppercase', letterSpacing: '0.06em',
                                    transition: 'all 0.15s',
                                }}
                            >
                    ...
                    <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                        {rightPanel === 'detail' && (
                            <div style={{ flex: 1, overflowY: 'auto', padding: 16 }}>
                                {selectedEvent ? (
                                    <div className="animate-fade-in">
                                        ...
                                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 16 }}>
                                            <div className="hud-border" style={{ padding: '10px 12px', background: 'var(--bg-card)' }}>
                                                <span className="font-mono" style={{ fontSize: '0.45rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Data</span>
                                                <div className="font-mono" style={{ fontSize: '0.75rem', color: 'var(--text-primary)', fontWeight: 600 }}>{selectedEvent.date}</div>
                                            </div>
                                            <div className="hud-border" style={{ padding: '10px 12px', background: 'var(--bg-card)' }}>
                                                <span className="font-mono" style={{ fontSize: '0.45rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Afetados</span>
                                                <div className="font-mono" style={{ fontSize: '0.75rem', color: 'var(--primary)', fontWeight: 600 }}>{selectedEvent.affected?.toLocaleString('pt-BR')}</div>
                                            </div>
                                            <div className="hud-border" style={{ padding: '10px 12px', background: 'var(--bg-card)' }}>
                                                <span className="font-mono" style={{ fontSize: '0.45rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Status</span>
                                                <div className="font-mono" style={{ fontSize: '0.75rem', color: 'var(--cyan)', fontWeight: 600 }}>{selectedEvent.status}</div>
                                            </div>
                                            <div className="hud-border" style={{ padding: '10px 12px', background: 'var(--bg-card)' }}>
                                                <span className="font-mono" style={{ fontSize: '0.45rem', color: 'var(--text-muted)', textTransform: 'uppercase' }}>Coordenadas</span>
                                                <div className="font-mono" style={{ fontSize: '0.65rem', color: 'var(--text-muted)', fontWeight: 500 }}>
                                    ...
                                    <div className="font-mono" style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)', fontSize: '0.6rem' }}>
                        ...
                        {rightPanel === 'ai' && <TacticalAI data={filteredData} />}
